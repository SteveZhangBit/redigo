...
func NewFromFloat64(val float64) *RString {
	return &RString{Val: []byte(fmt.Sprintf("%.17f", val))}
}
...
func (r *RString) Append(b []byte) {
	switch x := r.Val.(type) {
	case int64:
		r.Val = append([]byte(strconv.FormatInt(x, 10)), b...)
	case []byte:
		r.Val = append(x, b...)
	}
}
...
func INCRBYFLOATCommand(c *redigo.RedigoClient) {
	...
	if x, ok := GetFloat64FromStringOrReply(c, o, ""); !ok {
		...
		if o != nil {
			str = o.(*RString)
			str.Val = []byte(fmt.Sprintf("%.17f", x))
		} else {
